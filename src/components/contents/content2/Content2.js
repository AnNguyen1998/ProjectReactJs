          ...
          categories && categories.map((item, index) => (
            <Col key={index} lg={2} md={4} sm={6} xs={12} className='p-3'>
              <div style={{overflow:'hidden', borderRadius: '20px', position:'relative',
                ...
                <a href={'#'+item.name}>
                <img className='img-cate' src={item.image} style={{ width: '100%', borderRadius:'20px' }} />
                <p className='animate__animated animate__jello animate__slow animate__infinite' style={{position:'absolute', top:'10px', left:'20px', fontSize:'20px'}}>{item.name}</p>
                </a>
              </div>
